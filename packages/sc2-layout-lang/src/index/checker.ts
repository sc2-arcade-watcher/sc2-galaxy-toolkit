...
export class LayoutChecker {
    ...
    protected checkPropertyBindContext(cDesc: DescNamespace, el: XMLElement, nattr: XMLAttr, sType: sch.SimpleType) {
        switch (el.lang.sdef.nodeKind) {
            ...
            {
                if (!el.lang.stype.attributes.has('val')) {
                    this.reportAtAttrVal(
                        nattr,
                        `Property binding isn't supported for field type: "${el.lang.stype.name}"`
                    );
                    return false;
                }
            }
            ...
            default:
            {
                this.reportAtAttrVal(
                    nattr,
                    `Possible property binding used in an usupported context: "${sch.ElementDefKind[el.lang.sdef.nodeKind]}"`,
    ...
    protected checkFrameHookups(el: XMLElement, ufNode: FrameNode) {
        const sFrameType = this.store.schema.getFrameType(el.lang.stype);
    ...
    protected checkElement(el: XMLElement) {
        ...
        outer: for (const atName in el.attributes) {
            ...
            const vstype = el.lang.stype.attributes.get(atName);
            if (vstype) {
                asType = vstype.type;
            }

            if (!asType) {
                for (const [indKey, indItem] of el.lang.stype.indeterminateAttributes) {
                ...
                if (!asType) {
                    if (!(el.lang.stype.flags & sch.ComplexTypeFlags.AllowExtraAttrs)) {
        ...
        //
        if (el.lang.stype.indeterminateAttributes.size) {
            for (const indAttr of el.lang.stype.indeterminateAttributes.values()) {
