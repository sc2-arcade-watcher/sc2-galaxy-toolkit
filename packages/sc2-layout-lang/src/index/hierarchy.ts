        ...
        for (const currDesc of Array.from(this.descs).reverse()) {
            for (const xDecl of currDesc.xDecls) {
                for (const xCurrEl of xDecl.children) {
                    ...
                    let sTypeList = this._elementsMap.get(xCurrEl.lang.stype);
                    if (!sTypeList) {
                        sTypeList = [];
                        this._elementsMap.set(xCurrEl.lang.stype, sTypeList);
                    }
...
export class StateGroupNode extends UINode {
    get defaultState() {
        const defs = this.findElements(item => item.lang.sdef.nodeKind === sch.ElementDefKind.StateGroupDefaultState);
    ...
    get states() {
        ...
        for (const item of this.findElements(item => item.lang.sdef.nodeKind === sch.ElementDefKind.StateGroupState)) {
...
export class AnimationNode extends UINode {
    getEvents() {
        const r = new Map<string, XMLElement[]>();
        for (const item of this.findElements(item => item.lang.sdef.nodeKind === sch.ElementDefKind.AnimationEvent)) {
