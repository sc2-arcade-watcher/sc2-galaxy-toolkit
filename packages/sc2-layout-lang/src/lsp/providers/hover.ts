...
export class HoverProvider extends AbstractProvider {
    ...
    protected processElement(node: XMLElement) {
        ...
        let contents = `**&lt;${node.lang.sdef.name}&gt;**`;
        let dLink = '#';

        switch (node.lang.sdef.nodeKind) {
            case sch.ElementDefKind.Frame:
            {
                const sFrameType = this.store.schema.getFrameType(node.lang.stype);
                if (sFrameType) {
                    dLink = docsLink('frame-type', sFrameType.name);
            ...
            default:
            {
                contents += ` — ${labeledTypeLink(node.lang.sdef.type)}`;
                break;
            }
        }

        if (node.lang.sdef.label) {
            contents += '\n\n' + node.lang.sdef.label;
        }
        if (node.lang.stype.label) {
            contents += `\n\n` + node.lang.stype.label;
        }
        contents += '\n\n' + Array.from(node.lang.stype.attributes.values()).map(v => {
    ...
    protected processAttributeName(scAttr: sch.Attribute, xEl: XMLElement): lsp.Hover {
        ...
        contents.push(`Attribute of **&lt;${xEl.lang.sdef.name}&gt;** — `);
        if (xEl.lang.sdef.nodeKind === sch.ElementDefKind.Frame) {
            const sFrameType = this.store.schema.getFrameType(xEl.lang.stype);
            if (sFrameType) {
                contents.push(`[${sFrameType.name}](${docsLink('frame-type', sFrameType.name)})`);
            }
            else {
                contents.push(labeledTypeLink(xEl.lang.sdef.type));
            }
        }
        else {
            contents.push(labeledTypeLink(xEl.lang.sdef.type));
    ...
    protected processAttributeValue(scAttr: sch.Attribute, context: { xDoc: XMLDocument, position: lsp.Position }): lsp.Hover | undefined {
        switch (scAttr.type.builtinType) {
            default:
            {
                const wordRange = context.xDoc.lang.tdoc.getWordRangeAtPosition(context.position, this.slSrv.wordPattern);
                if (!wordRange) break;
                const flatEnum = this.store.schema.flattenSTypeEnumeration(scAttr.type);
                const matchedEn = flatEnum.get(context.xDoc.lang.tdoc.getText(wordRange));
    ...
    async provideHover(params: lsp.TextDocumentPositionParams, token: lsp.CancellationToken) {
        ...
        if (node instanceof XMLElement && node.lang.stype) {
            if (node.start <= offset && (node.start + node.tag.length + 1) > offset) {
                ...
                if (attr) {
                    let scAttr = node.lang.stype.attributes.get(attr.name.toLowerCase());
            ...
            switch (defContainer.itemKind) {
                ...
                case DefinitionItemKind.XNode:
                {
                    ...
                    for (const xEl of (<DefinitionXNode>defContainer.itemData).xNodes) {
                        ...
                        mstr.push({
                            language: 'sc2layout',
                            value: xEl.getDocument().lang.tdoc.getText(<lsp.Range>vsRangeOrPositionOfXNode(xEl)),
                        });
                    }
                    hv = {
                        contents: mstr,
