...
class AttrValueProvider extends SuggestionsProvider {
        ...
        switch (smType) {
            ...
            case sch.BuiltinTypeKind.FrameReference:
            {
                ...
                if (
                    ...
                    (
                        ctx.node?.lang.sdef.nodeKind === sch.ElementDefKind.StateGroupStateAction &&
    ...
    protected suggestDescNames(ctx: AtValComplContext) {
        ...
        if (!xElParent.lang.sdef) return;
        switch (xElParent.lang.sdef.nodeKind) {
    ...
    protected suggestEventNames(ctx: AtValComplContext, sAttrType: sch.SimpleType) {
        switch (ctx.node.lang.sdef.nodeKind) {
    ...
    protected suggestPropertyNames(ctx: AtValComplContext) {
        ...
        if (uFrame && uFrame.mainDesc) {
            const sfType = this.store.schema.getFrameType(Array.from(uFrame.mainDesc.xDecls)[0].lang.stype);
            if (sfType) {
    ...
    public provide(ctx: AtValComplContext) {
        const sAttrItem = ctx.node.lang.stype.attributes.get(ctx.attrNameLower);
        ...
        if (this.store.schema.isPropertyBindAllowed(ctx.node.lang.sdef, ctx.node.lang.stype, ctx.attrName) && pvKind === AttrValueKind.PropertyBind) {
...
class AttrNameProvider extends SuggestionsProvider {
    protected matchIndeterminateAttr(ctx: AtComplContext) {
        if (!ctx.node.lang.stype.indeterminateAttributes.size) return;

        for (const [sname, sattr] of ctx.node.lang.stype.attributes) {
            if (!sattr.required) continue;
            if (!ctx.node.attributes[sname]) return;
        }

        for (const atKey in ctx.node.attributes) {
            if (ctx.node.lang.stype.attributes.has(atKey)) continue;
        ...
        return Array.from(ctx.node.lang.stype.indeterminateAttributes.values())[0];
    }
    ...
    public provide(ctx: AtComplContext) {
        ...
        for (const [sAttrKey, sAttrItem] of ctx.node.lang.stype.attributes) {
            if (
        ...
        if (indAttr) {
            ...
            switch (indAttr.key.builtinType) {
                ...
                case sch.BuiltinTypeKind.PropertyName:
                {
                    ...
                    if (uFrame && uFrame.mainDesc) {
                        sfType = this.store.schema.getFrameType(Array.from(uFrame.mainDesc.xDecls)[0].lang.stype);
                        if (sfType) {
                            for (const currScProp of sfType.fprops.values()) {
...
export class CompletionsProvider extends AbstractProvider {
    ...
    protected suggestAnchors(ctx: ComplContext, nodeCtx: XMLElement) {
        const scElAnchor = nodeCtx.lang.stype.struct.get('Anchor');
    ...
    protected suggestStateGroup(ctx: ComplContext, nodeCtx: XMLElement) {
        ...
        const itemSch = nodeCtx.lang.stype.struct.get(category);
        for (let l = 1; l <= 5; ++l) {
            const complItem = <lsp.CompletionItem>{
                label: `${category}:${l}`,
                kind: lsp.CompletionItemKind.Interface,
    ...
    protected suggestStateGroupInstruction(ctx: ComplContext, nodeCtx: XMLElement) {
        for (const category of ['When', 'Action']) {
            for (const [itemType, itemSch] of nodeCtx.lang.stype.struct.get(category).altType.statements) {
                const complItem = <lsp.CompletionItem>{
    ...
    protected suggestElements(ctx: ComplContext) {
        ...
        if (!nodeCtx.lang.stype) return;

        const isNewTag =
            ((ctx.xtoken === TokenType.Content || ctx.xtoken === TokenType.StartTagOpen) || !ctx.node.closed) &&
            [CharacterCodes.greaterThan].find(v => v === ctx.node.getDocument().lang.tdoc.getText().charCodeAt(ctx.offset)) === void 0
        ;

        if (isNewTag) {
            switch (nodeCtx.lang.sdef.nodeKind) {
                case sch.ElementDefKind.Frame:
        ...
        for (const [sElKey, sElItem] of nodeCtx.lang.stype.struct) {
            const complItem = <lsp.CompletionItem>{
                label: sElKey,
    ...
    async provideCompletionItems(params: lsp.CompletionParams, cancToken: lsp.CancellationToken): Promise<lsp.CompletionList> {
        ...
        const offset = sourceFile.lang.tdoc.offsetAt(params.position);
        ...
        if (node) {
            ...
            startOffset = sourceFile.lang.tdoc.offsetAt(sourceFile.lang.tdoc.getWordRangeAtPosition(params.position).start);
        }

        // console.log('offset', offset);
        let scanner = createScanner(sourceFile.lang.tdoc.getText(), startOffset);
        ...
        if (scanner.getScannerState() === ScannerState.AfterOpeningEndTag || scanner.getScannerState() === ScannerState.WithinEndTag) {
            items.push(<lsp.CompletionItem>{
                ...
                insertText: node.tag + (sourceFile.lang.tdoc.getText().charCodeAt(scanner.getTokenEnd()) === CharacterCodes.greaterThan ? '' : '>'),
        ...
        switch (token) {
            ...
            case TokenType.Content:
            {
                const abbrvRange = sourceFile.lang.tdoc.getWordRangeAtPosition(params.position, reAbbrvWord);
                if (abbrvRange && token === TokenType.Content) {
                    const abrvResult = this.codeAbrvProvider.processAbbrv({
                        vDoc: sourceFile.lang.tdoc,
                        xEl: node,
