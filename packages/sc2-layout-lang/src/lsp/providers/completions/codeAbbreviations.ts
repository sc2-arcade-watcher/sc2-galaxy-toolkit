    ...
    protected provideDescOverrides(ctx: AbbrComplContext) {
        ...
        if (pathIndex >= 2) {
            ...
            if (pathIndex === pathSel.path.length || pathSel.path[pathSel.path.length - 1].pos === pathSel.path[pathSel.path.length - 1].end) {
                ...
                let rawTagName = xTargetEl.lang.sdef.name;
                let rawAttrType = uTargetNode.mainDesc.kind === DescKind.Frame ? ` type="${xTargetEl.lang.stype.name}"` : '';
                ...
                cList.items.push({
                    ...
                    detail: `${uTargetNode.name}[${(<XMLElement>Array.from(uTargetNode.mainDesc.xDecls)[0]).lang.sdef.name}]`,
