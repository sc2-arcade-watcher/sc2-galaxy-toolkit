...
export function vsRangeOrPositionOfXNode(xNode: XMLNode) {
    ...
    switch (xNode.kind) {
        case XMLNodeKind.Document:
        {
            ...
            if (!rootNode) {
                const posSta = xDoc.lang.tdoc.positionAt(xEl.start);
    ...
    const posSta = xDoc.lang.tdoc.positionAt(xEl.start);
    const posEnd = xDoc.lang.tdoc.positionAt(xEl.startTagEnd ? xEl.startTagEnd : xEl.end);
    return lsp.Range.create(
        lsp.Position.create(posSta.line, posSta.character),
...
export function vsLocationOfXEl(xEl: XMLNode) {
    return lsp.Location.create(xEl.getDocument().lang.tdoc.uri, vsRangeOrPositionOfXNode(xEl));
}

export function getAttrInfoAtPosition(xDoc: XMLDocument, offset: number) {
    const xEl = <XMLElement>xDoc.findNodeAt(offset);
    if (!xEl || !(xEl instanceof XMLElement) || !xEl.lang.stype) return;
    if (xEl.closed) {
    ...
    const sAttrType = xEl.lang.stype.attributes.get(xAttrNameLower);
